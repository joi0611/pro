...
const resumableScreens=["review","scan","study","test","reinforce","reinforceWord","round2"];
function checkpoint(){
  ...
  profile.activeSession={
    ...
    round2Group:state.round2Group,round2Idx:state.round2Idx,round2Queue:state.round2Queue.map(x=>x.w),round2Results:state.round2Results.map(x=>({w:x.word?.w||x.w,category:x.category||""})),round2Mistakes:state.round2Mistakes.map(m=>({w:m.word?.w||m.w,picked:m.picked,answer:m.answer})),round2PartialFrom:state.round2PartialFrom||0,
...
function restoreCheckpoint(){
  ...
  state.round2Group=s.round2Group||0;state.round2Idx=s.round2Idx||0;state.round2Queue=(s.round2Queue||[]).map(wordByName).filter(Boolean);state.round2Results=(s.round2Results||[]).map(v=>({word:wordByName(v.w),category:v.category||""})).filter(v=>v.word);state.round2Mistakes=(s.round2Mistakes||[]).map(v=>({word:wordByName(v.w),picked:v.picked,answer:v.answer})).filter(v=>v.word);state.round2PartialFrom=s.round2PartialFrom||0;
...
function home(){
  ...
  const stats=profile.dailyStats[active],r2today=round2PlanFor(active),done=(stats.new||0)+(stats.review||0)+(r2today?.done||0),total=(stats.planNew||0)+(stats.planReview||0)+(r2today?.count||0),percent=Math.min(100,done/(total||1)*100),finished=isDayComplete(active),hasResume=profile.activeSession&&profile.activeSession.date===studyDate()&&resumableScreens.includes(profile.activeSession.screen),reviewText=reviewEnabled()?`艾宾浩斯计划 · ${stats.planReview} 词`:"只学新词模式 · 0 词",todayReviewCount=todayContentWords(active).length;
  ...
  <div class="dashboard-grid"><section class="panel calendar-panel">${calendar()}</section><section class="panel task-panel"><div class="label">学习任务</div><div class="task-line"><span class="task-icon review-icon">↻</span><div><b>记忆复习</b><small>${reviewText}</small></div><strong>${stats.review||0}/${stats.planReview||0}</strong></div><div class="task-line"><span class="task-icon new-icon">＋</span><div><b>学习新词</b><small>先筛词，后学习</small></div><strong>${stats.new||0}/${stats.planNew}</strong></div><button class="primary ${finished&&!hasResume?"day-finished":""}" id="today">${hasResume?"继续学习":finished?"今日学完 ✓":r2today&&!finished?"继续二轮检测":due?"先复习，再开始学新":"开始筛词"}</button>${learningRouteComplete()?'<button class="today-review-btn" id="round2Start">'+((hasResume&&profile.activeSession.screen==="round2")||r2today?'继续二轮检测':'开始第二轮检测')+'</button><p class="advance-tip">第一轮词汇学习已完成。接下来建议进入第二轮检测，重点找出“看过但不熟”的词。</p>':finished&&!hasResume&&todayReviewCount?'<button class="today-review-btn" id="todayReview">复习当日内容</button>':""}${finished&&!hasResume?'<p class="advance-tip">双击日历中的下一日，可以提前学习</p>':""}</section></div>
  ...
  document.querySelector("#today").onclick=()=>{if(profile.activeSession&&profile.activeSession.date===studyDate()&&resumableScreens.includes(profile.activeSession.screen)&&restoreCheckpoint()){render();return}if(finished){alert("今日任务已完成，双击日历中的下一日即可提前学习。");return}if(r2today&&!finished){state.screen="round2";render();return}if(due){state.reviewIdx=0;state.reveal=false;state.reviewQueue=dueReviews(active);state.reviewTotal=state.reviewQueue.length;state.screen="review";render()}else startScan()};
  const todayReviewButton=document.querySelector("#todayReview");if(todayReviewButton)todayReviewButton.onclick=()=>{state.todayReviewMode="en-cn";state.todayReviewRevealed=[];state.screen="todayReview";render()};const round2Button=document.querySelector("#round2Start");if(round2Button)round2Button.onclick=()=>{if(profile.activeSession&&profile.activeSession.date===studyDate()&&profile.activeSession.screen==="round2"&&restoreCheckpoint()){render();return}if(r2today&&!state.round2Queue.length){state.round2Group=r2today.group;state.round2Queue=shuffle(round2Groups()[r2today.group]||[])}else if(!r2today){state.round2Group=0;state.round2Idx=0;state.round2Queue=shuffle((round2Groups()[0]||[]));state.round2Results=[];state.round2Answer="";state.round2Feedback=""}scheduleRound2Group(state.round2Group);state.screen="round2";render()};
...
function calendar(){
  ...
  return `<div class="calendar-head"><div><div class="label">学习日历</div><h2>${year} 年 ${month+1} 月</h2></div><div class="legend"><i></i> 已完成</div></div><p class="calendar-tip">${reviewEnabled()?"复习数按学新后的第 1、4、7、15 天自动叠加":"当前为只学新词模式，复习数从今天起显示为 0"}</p><div class="week">${"一二三四五六日".split("").map(x=>`<b>${x}</b>`).join("")}${cells.map(d=>{if(!d)return "<span></span>";let key=`${year}-${String(month+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`,s=dayPlan(key),reviewPlan=calendarReviewPlan(key),r2=round2PlanFor(key),classes=[key===today()?"today-day":"",key===active?"active-day":"",profile.history[key]||"",key===next&&canAdvance?"available-next":""].join(" ");return `<span class="calendar-day ${classes}" data-date="${key}"><b>${d}</b><small>新 ${s.new||0}/${s.planNew}</small><small>复 ${s.review||0}/${reviewPlan}</small>${r2?`<small>二测 ${r2.group+1}组 ${r2.done}/${r2.count}</small>`:""}</span>`}).join("")}</div>${crossMonth&&canAdvance?`<button class="calendar-next available-next" data-date="${next}">双击进入 ${nextDate.getMonth()+1}月${nextDate.getDate()}日 →</button>`:""}`;
}
function settings(){
...
function isDayComplete(key){let s=dayPlan(key),r2=round2PlanFor(key);return s.new>=s.planNew&&s.review>=s.planReview&&(!r2||r2.done>=r2.count)}
...
function learningRouteComplete(){ensureCompletionData();const route=learningLibraries();return route.length>0&&route.every(lib=>(profile.completedWords||[]).filter(k=>k.startsWith(`${lib}:`)).length>=banks[lib].length)}
function round2Words(){ensureCompletionData();return learningLibraries().flatMap(lib=>banks[lib]).filter(Boolean)}
function round2Groups(){const words=round2Words(),groups=[];for(let i=0;i<words.length;i+=100)groups.push(words.slice(i,i+100));return groups}
function scheduleRound2Group(groupIndex,key=studyDate(),count=100){profile.round2=profile.round2||{results:{},groups:{},mastered:[],schedule:{}};profile.round2.schedule=profile.round2.schedule||{};if(!profile.round2.schedule[key])profile.round2.schedule[key]={group:groupIndex,count,status:"planned"};profile.dailyStats=profile.dailyStats||{};const s=profile.dailyStats[key]||{new:0,review:0};s.round2Group=groupIndex;s.round2Plan=Math.min(count,MAX_DAILY_WORDS);s.round2Done=0;profile.dailyStats[key]=s;save()}
function round2PlanFor(key){const s=profile.dailyStats?.[key];return s?.round2Plan?{group:s.round2Group,count:s.round2Plan,done:s.round2Done||0}:null}
function round2Resume(){const r2=round2PlanFor(studyDate()),groups=round2Groups();if(!r2||!groups[r2.group])return false;state.round2Group=r2.group;if(!state.round2Queue.length)state.round2Queue=shuffle(groups[r2.group]);state.round2Results=[];state.round2Mistakes=[];state.round2Answer="";state.round2Feedback="";state.screen="round2";render();return true}
function round2Start(){const groups=round2Groups();if(!groups.length)return;state.round2Group=0;state.round2Idx=0;state.round2Queue=shuffle(groups[0]);state.round2Results=[];state.round2Mistakes=[];state.round2Answer="";state.round2Feedback="";state.round2PartialFrom=0;scheduleRound2Group(0);state.screen="round2";render()}
function round2Last10(){const groups=round2Groups();if(!groups.length)return;let gi=groups.length-1;while(gi>0&&(groups[gi]||[]).length<10)gi--;const group=groups[gi]||[],start=Math.max(0,group.length-10);state.round2Group=gi;state.round2Idx=start;state.round2Queue=shuffle(group);state.round2Results=[];state.round2Mistakes=[];state.round2Answer="";state.round2Feedback="";state.round2PartialFrom=start;scheduleRound2Group(gi);state.screen="round2";render()}
function round2ReviewStart(){const groups=round2Groups(),group=groups[state.round2Group]||[];if(!group.length)return;state.round2Idx=0;state.round2Queue=shuffle(group);state.round2Results=[];state.round2Mistakes=[];state.round2Answer="";state.round2Feedback="";state.screen="round2";render()}
function round2Question(x){const useMeaning=(state.round2Idx+state.round2Group)%2===0;if(useMeaning){return {mode:"meaning",text:x.w,answer:shortCn(x),options:meaningOptions(x),label:"选择最合适的中文意思"}}const answer=x.w;return {mode:"word",text:shortCn(x),answer,options:[answer,...distractors(x).filter(v=>v.toLowerCase()!==answer.toLowerCase())].slice(0,4),label:"选择对应的英文单词"}}
function scheduleRound2Review(x,category){profile.reviews=profile.reviews||[];const due=category==="熟练词"?addDate(studyDate(),7):addDate(studyDate(),1),level=category==="熟练词"?2:0;const existing=profile.reviews.find(r=>r.w===x.w);if(existing){existing.due=existing.due<due?existing.due:due;existing.level=Math.max(existing.level||0,level)}else profile.reviews.push({w:x.w,level,due});ensureDayPlan(due,scheduledReviewCount(due),true)}
function round2SaveResult(x,ok){profile.round2=profile.round2||{results:{},groups:{},mastered:[]};const key=completedKey(x),old=profile.round2.results[key]||{correct:0,wrong:0,streak:0};if(ok){old.correct++;old.streak=(old.streak||0)+1}else{old.wrong++;old.streak=0}old.lastDate=studyDate();profile.round2.results[key]=old;const mastered=old.streak>=2;if(mastered&&!profile.round2.mastered.includes(key))profile.round2.mastered.push(key);const category=round2Category(x);if(category==="熟练词")scheduleRound2Review(x,category);if(!ok)scheduleRetry(x.w);save()}
function round2Category(x){const r=profile.round2?.results?.[completedKey(x)]||{correct:0,wrong:0};return r.wrong>0?"混淆词":"熟练词"}
function round2(){const groups=round2Groups();if(!groups.length){state.screen="home";return render()}const group=groups[state.round2Group]||[];if(state.round2Idx>=group.length){const from=state.round2PartialFrom||0,part=(state.round2Queue||group).slice(from);profile.round2=profile.round2||{results:{},groups:{},mastered:[]};profile.round2.groups[state.round2Group]={date:studyDate(),count:group.length};save();state.round2Results=part.map(x=>({word:x,category:round2Category(x)}));state.round2Mistakes=part.filter(x=>round2Category(x)==="混淆词").map(x=>{const saved=state.round2Mistakes.find(m=>m.word?.w===x.w);return {word:x,picked:saved?.picked||"",answer:saved?.answer||""}});state.screen="round2Summary";render();return}const x=state.round2Queue[state.round2Idx],q=round2Question(x),opts=shuffle([...new Set(q.options)]).slice(0,4);app.innerHTML=shell(`<section class="panel quiz"><div class="study-meta"><span>第二轮检测 · 第 ${state.round2Group+1} 组 / ${groups.length} 组</span><span>${state.round2Idx+1} / ${group.length}</span></div>${progressBar((state.round2Idx+1)/group.length*100)}<div class="quiz-body"><div class="label">${q.label}</div><h2 class="cloze-sentence">${q.text}</h2><div class="options word-options ${q.mode==="meaning"?"meaning-options":""}">${opts.map(o=>`<button data-round2-answer="${o===q.answer}">${o}</button>`).join("")}</div><div class="feedback ${state.round2Feedback?"wrong":""}">${state.round2Feedback}</div></div></section>`,true);bindHome();document.querySelectorAll("[data-round2-answer]").forEach(b=>b.onclick=()=>{if(state.round2Answer)return;const ok=b.dataset.round2Answer==="true",picked=b.textContent.trim();state.round2Answer=q.answer;state.round2Feedback=ok?"✓ 回答正确":`✗ 正确答案：${q.answer}`;if(!ok){const ex=state.round2Mistakes.find(m=>m.word?.w===x.w);if(ex){ex.picked=picked;ex.answer=q.answer}else state.round2Mistakes.push({word:x,picked,answer:q.answer})}round2SaveResult(x,ok);const r2=profile.dailyStats?.[studyDate()];if(r2?.round2Group===state.round2Group){r2.round2Done=Math.min(r2.round2Plan||0,(r2.round2Done||0)+1);save()}setTimeout(()=>{state.round2Idx++;state.round2Answer="";state.round2Feedback="";round2()},ok?500:1600)})}
function round2Summary(){const groups=round2Groups(),items=state.round2Results||[],counts={"熟练词":0,"混淆词":0};items.forEach(x=>counts[x.category]++);const mistakes=state.round2Mistakes||[],confuseCount=mistakes.length,errorRate=items.length?confuseCount/items.length:0,isLast=state.round2Group>=groups.length-1,advice=isLast?`本轮全部 ${groups.length} 组已检测完成！${confuseCount?`共 ${confuseCount} 个混淆词，请按选择重新学习或安排到明天。`:"所有词都掌握，太棒了！"}`:errorRate<.25?`本组混淆词占 ${Math.round(errorRate*100)}%，低于 25%。建议明天继续检测第 ${state.round2Group+2} 组。`:`本组混淆词占 ${Math.round(errorRate*100)}%，达到或超过 25%。请先完成混淆词学习，再主动进行二轮检测。`;app.innerHTML=shell(`<section class="panel"><div class="memory-kicker">第二轮检测完成 · 第 ${state.round2Group+1} 组 / ${groups.length} 组</div><h2>这一组检测结果</h2><p class="muted">本轮检测 ${items.length} 词</p><div class="round2-stats">${Object.entries(counts).map(([k,v])=>`<div><b>${v}</b><span>${k}</span></div>`).join("")}</div><div class="advice"><b>${advice}</b></div>${mistakes.length?`<div class="label section-gap">混淆词回顾</div><div class="round2-mistakes">${mistakes.map(m=>`<div class="round2-mistake-row"><b>${m.word?.w||m.w}</b><span>你选了：${m.picked||"—"}</span><span class="right">正确答案：${m.answer}</span></div>`).join("")}</div><div class="round2-actions"><button class="primary" id="round2LearnToday">今日重新学习混淆词</button><button class="secondary" id="round2LearnTomorrow">明日再学</button></div>`:`<p class="muted">本组没有混淆词，全部掌握！</p>`}<p class="muted">熟练词后续降低出现频率；混淆词按你的选择重新学习或安排到明天。</p><button class="secondary" id="round2Review">主动进行二轮检测</button>${state.round2Group<groups.length-1?`<button class="primary" id="nextRound2">明天继续第 ${state.round2Group+2} 组检测</button>`:`<button class="primary" id="finishRound2">完成本轮复习</button>`}</section>`,true);bindHome();const learnToday=document.querySelector("#round2LearnToday");if(learnToday)learnToday.onclick=()=>{if(!mistakes.length)return;const words=mistakes.map(m=>m.word).filter(Boolean);profile.relearn=(profile.relearn||[]).filter(r=>!words.some(w=>w.w===r.w));save();state.queue=words;state.idx=0;resetStudyTracking();state.screen="study";render()};const learnTomorrow=document.querySelector("#round2LearnTomorrow");if(learnTomorrow)learnTomorrow.onclick=()=>{mistakes.forEach(m=>{if(m.word?.w)scheduleRetry(m.word.w)});save();state.screen="home";render()};const review=document.querySelector("#round2Review");if(review)review.onclick=round2ReviewStart;const next=document.querySelector("#nextRound2");if(next)next.onclick=()=>{state.round2Group++;state.round2Idx=0;state.round2Queue=shuffle(groups[state.round2Group]);state.round2Results=[];state.round2Mistakes=[];scheduleRound2Group(state.round2Group,addDate(studyDate(),1));state.screen="round2";render()};const finish=document.querySelector("#finishRound2");if(finish)finish.onclick=()=>{state.screen="home";render()}}
...
function render(){if(profile&&resumableScreens.includes(state.screen))checkpoint();({login,onboard,home,settings,review,todayReview,scan,study,test,reinforce:reinforceIntro,reinforceWord:reinforceIntro,round2,round2Summary}[state.screen]||home)();bindLogout()}
async function boot(){
